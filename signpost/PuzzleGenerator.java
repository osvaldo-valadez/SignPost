    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place a: sq.successors()) {
                    if (sq.sequenceNum() != 0
                            && model.get(a.x, a.y).sequenceNum() != 0
                            && sq.connectable(model.get(a.x, a.y))) {
                        nFound = 1;
                        found = model.get(sq);
                        break;
                    }
                    if (sq.connectable(model.get(a.x, a.y))) {
                        nFound += 1;
                        found = model.get(a.x, a.y);
                    }
                }
            }
            if (nFound == 0) {
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place a: sq.predecessors()) {
                    if (sq.sequenceNum() != 0
                            && model.get(a.x, a.y).connectable(sq)) {
                        nFound = 1;
                        found = model.get(sq);
                        break;
                    }

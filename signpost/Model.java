...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                int numOfPlace = solution[x][y];
                _solnNumToPlace[numOfPlace] = pl(x, y);
            }
        }
        ...
        PlaceList[][][] sucopy = Place.successorCells(_width, _height);
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                _board[x][y]._successors = sucopy[x][y][arrowDirection(x, y)];
            }
        }
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                for (Place p : _board[x][y]._successors) {
                    if (get(p).predecessors() == null) {
                        get(p)._predecessors = new PlaceList();
                    }
                    get(p)._predecessors.add(pl(x, y));
                }
            }
        }
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                if (model._board[x][y]._successor == null) {
                    ...
                    _board[x][y]._successor = _board[x1][y1];
                }
                if (model._board[x][y]._predecessor == null) {
                    ...
                    _board[x][y]._predecessor = _board[x2][y2];
                }
                int x3 = model._board[x][y]._head.x;
                int y3 = model._board[x][y]._head.y;
                _board[x][y]._head = _board[x3][y3];
            }
        }
        this._board = _board;
    }
    ...
    void solve() {
        for (int x = 0; x < width(); x++) {
            for (int y = 0; y < height(); y++) {
                Sq loc = get(x, y);
                loc._sequenceNum = _solution[x][y];
            }
        }
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        for (int x1 = 0; x1 < _width; x1++) {
            for (int y1 = 0; y1 < _height; y1++) {
                if (_solution[x1][y1] == _solution[x][y] + 1) {
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (dirOf(this.x, this.y, s1.x, s1.y) == this.direction()) {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() != 0) {
                Sq current = this;
                while (current._successor != null) {
                    current._head = this._head;
                    current._successor._sequenceNum = current.sequenceNum() + 1;
                ...
                while (current._predecessor != null) {
                    current._head = this._head;
                    current._predecessor._sequenceNum =
                            current.sequenceNum() - 1;
                    current = current._predecessor;
                }
            } else if (this._group == 0 && s1._group != 0
                    || s1._group == 0 && this._group != 0) {
                if (this._group != 0) {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null
                        && this._successor == null && next._predecessor == null
                        && next._successor == null) {
                    releaseGroup(this._group);
                    releaseGroup(next._group);
                    this._group = -1;
                    next._group = -1;
                } else if (this._predecessor == null  && this._successor
                        == null || next._predecessor == null
                        && next._successor == null) {
                    if (this._predecessor == null) {
                        this._group = -1;
                    } else {
                ...
                } else if (temp._group != 0) {
                    while (this._group == temp._group) {
                        temp.setFixedNum(0);
                ...
                if (temp2._successor == null) {
                    if (temp2._group != 0) {
                        temp2.setFixedNum(0); temp2._group = -1;
                    }
                } else if (temp2._group != 0) {
                    while (this._group == temp2._group) {
                        temp2.setFixedNum(0);
                        temp2 = temp2._successor;
                    }
                    temp2._group = newGroup();
                }
            }
            Sq temp3 = next;
            while (temp3._successor != null) {
                temp3._head = next;
